...
function InstanceLayer(webSocket, connection, modelDirectory, currentUserId) {
  ...
  this.webSocket.on("LiveDocumentEmbeddedCreate", this.handleEmbeddedCreateMessage.bind(this))
  this.webSocket.on("LiveDocumentEmbeddedDelete", this.handleEmbeddedDeleteMessage.bind(this))
  ...
  this.internalSocket.on("EmbeddedLiveDocumentCreate", function(parentCollection, parentId, collection, doc) {
    that.webSocket.emit("EmbeddedLiveDocumentCreate", parentCollection, parentId, collection, doc._id, doc)   
  })
  this.internalSocket.on("EmbeddedLiveDocumentRemove", function(parentCollection, parentId, collection, id) {
    that.webSocket.emit("EmbeddedLiveDocumentRemove", parentCollection, parentId, collection, id)   
  })
}
...
InstanceLayer.prototype.handleEmbeddedCreateMessage = function(parentCollection, parentId, collection, document, callback) {
  this.internalSocket.emit("LiveDocumentEmbeddedCreate", parentCollection, parentId, collection, document, callback)
}

InstanceLayer.prototype.handleEmbeddedDeleteMessage = function(parentCollection, parentId, collection, conditions, callback) {
  this.internalSocket.emit("LiveDocumentEmbeddedDelete", parentCollection, parentId, collection, conditions, callback)
}
