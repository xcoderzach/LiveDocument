...
function InstanceLayer(webSocket, connection) {
  ...
  this.webSocket.on("LiveDocumentCollectionRemove", this.handleDeleteMessage.bind(this))
}

InstanceLayer.prototype.handleReadMessage = function() {
  var that = this
  var args = [].slice.call(arguments)
  args.unshift("LiveDocumentRead")
  this.internalSocket.emit.apply(this.internalSocket, args)

  this.internalSocket.on("LiveDocument" + args[3], function() {
    try {
    var responseArgs = [].slice.call(arguments)
    responseArgs.unshift("LiveDocument" + args[3])
    that.webSocket.emit.apply(that.webSocket, responseArgs)
    } catch(e) {
      console.log(e.stack)
    }
  })
...
InstanceLayer.prototype.handleDeleteMessage = function() {
  this.internalSocket.emit.apply(
                          this.internalSocket
                        , [].slice.call(arguments).unshift("LiveDocumentCollectionRemove")
                                )
}
