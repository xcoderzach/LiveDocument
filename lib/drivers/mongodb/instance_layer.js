...
function InstanceLayer(webSocket, connection, modelDirectory) {
  ...
  this.webSocket.on("newListener", this.routeIdMessages.bind(this))
}

InstanceLayer.prototype.routeIdMessages = function(event, listener) {
  this.internalSocket.on(event, listener)
}

InstanceLayer.prototype.handleReadMessage = function(collection, document, requestNumber) {
  var that = this
  var Instance = require(this.modelDirectory + "/" + _.singularize(collection))
