...
function InstanceLayer(webSocket, connection, modelDirectory, currentUserId) {
  ...
  this.internalSocket.on("Change", function(id, doc, collection) {
    var Instance = require(that.modelDirectory + "/" + _.singularize(_.underscore(collection)))
    that.webSocket.emit("Change", id, Instance.scrub(doc, "read"))   
  })
  this.internalSocket.on("Remove", function(id, doc, collection) {
    var Instance = require(that.modelDirectory + "/" + _.singularize(_.underscore(collection)))
    that.webSocket.emit("Remove", id, Instance.scrub(doc, "read"))   
  })
...
InstanceLayer.prototype.handleUpdateMessage = function(collection, conditions, document, callback) {
  ...
  Instance.findOne(conditions, function(instance) {
    ...
    document = Instance.scrub(document, "update")
    instance.set(document)
    instance.save(function() {
      document = Instance.scrub(instance.document, "read")
      callback(document)
    })
  })
}
