...
LiveDocumentMongo.prototype.setSocket = function(socket) {
  ...
  this.socket.on("NotListening", this.stopListeningToId.bind(this))
...
LiveDocumentMongo.prototype.stopListeningToId = function(id) {
  this.ids[id] && this.ids[id].unwatch()
  this.embeddedIds[id] && this.embeddedIds[id].unwatch()
}

// Watches a particular id for updates or deletes

LiveDocumentMongo.prototype.handleDocumentChange = function(document, type) {
  this.socket.emit(_.capitalize(type), document._id, document)
}
