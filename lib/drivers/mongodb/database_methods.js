  ...
  module.exports = function(db) {
    ...
    , createEmbedded: function(parentCollection, parentDocumentId, collectionName, document, callback) {
      ...
      * @api private
      */
    , updateEmbedded: function(parentCollection, parentDocumentId, collectionName, id, document, callback) {
        var collection = db.collection(parentCollection)
          , condition = {}
          , update = {}
        condition[collectionName + '._id'] = id
        ...
        collection.findAndModify({ query: condition, update: { $set: update } }, function(err, oldDoc) {
          callback(document)
        })
      } 
    ...
    , removeEmbedded: function(parentCollection, parentDocumentId, collectionName, conditions, callback) {
        ...
        collection.findOne({ _id: parentDocumentId }, function(err, newDoc) {
          console.log(newDoc)
        })
