    ...
    _(this.constructor.hasMany).each(function(obj, collectionName) {
      ...
      that.manyAssociations[collectionName] = function(callback) {
        ...
        if(Model.embedded) {
          ...
          collection = new EmbeddedCollection(docs, Model, {parentType: that.collectionName, parentId: that.get("_id") })
        } else {
          query = {}
...
, setupBelongsToAssociations: function() {
    ...
    _(this.constructor.belongingTo).each(function(opts, collectionName) {
      that.belongAssociations[collectionName] = function(cb) {
        ...
        model.once("notExist", function(err) {
          throw new Error("Can't access non existant belongs to association.")
        })
        model.load(function() {
          cb(model)
        })
...
, getDb: function() {
    if(this.constructor.isServer) {
      if(this.embedded) {
        return DatabaseMethods(this.constructor.db).embedded(this.get("parentType"), this.get("parentId"), this.collectionName)
      } else {
        return DatabaseMethods(this.constructor.db).collection(this.collectionName)
      }
    } else {
      if(this.embedded) {
...
, set: function(field, value, opts) {
    ...
    if(typeof field == "object") {
      ...
      that.manyAssociations[field]().handleNotification(value, "load") 
    } else {
