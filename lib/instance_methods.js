...
define( ["require", "./object_id", "underscore", "./embedded_collection", "./utils"]
  ...
  var LiveDocumentInstanceMethods = {
    ...
    constructor: function(document, opts) {
      ...
      _(this.constructor.hasMany).each(function(Model, collectionName) {
        ...
        that.manyAssociations[collectionName] = new EmbeddedLiveDocumentCollection(docs, that, Model)
      })
      _(this.constructor.hasOne).each(function(Model, collectionName) {
        // don't find the assoc until the parent document is actually loaded
        if(this.loaded) {
          var query = {} 
          query[_.singularize(that.collectionName) + "Id"] = that.get("_id")
          that.oneAssociations[collectionName] = Model.findOne(query)
        } else {
          var query = {} 
          query[_.singularize(that.collectionName) + "Id"] = that.get("_id")
          that.oneAssociations[collectionName] = new Model(query)
        }
      })
  ...
  , assoc: function(assoc, callback) {
      var model = this.manyAssociations[assoc] || this.oneAssociations[assoc]
