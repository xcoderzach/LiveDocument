...
var InstanceMethods = {
  ...
  constructor: function(document, opts) {
    ...
    if(!this.document._id && !opts.noId) {
    ...
    this.polymorphicBelongAssociations = {}

    this.collectionName = this.constructor.collectionName()
    // Make this work for nonMany assocs
    ...
    _(this.constructor.hasOne).each(function(obj, collectionName) {
      ...
      that.oneAssociations[collectionName] = function(cb) {
        var query = {} 
        query[foreignKey] = that.get("_id")
        var model = Model.findOne(query)
    ...
    _(this.constructor.belongingTo).each(function(Model, collectionName) {
      that.belongAssociations[collectionName] = function(cb) {
        var foreignKey = that.get(collectionName + "Id")
        if(!foreignKey) {
          cb()
          return
        }
        var model = Model.findOne(that.get(collectionName + "Id"))
        //we couldn't find the model
        model.once("notExist", function(err) {
          model.save(function() {
            model.emit("load")
    ...
    _(this.constructor.polymorphicBelongingTo).each(function(ModelMap, assocName) {
      that.polymorphicBelongAssociations[assocName] = function(cb) {
        var foreignKey = that.get(assocName + "Id")
          , foreignType = that.get(assocName + "Type")
          , ForeignModel = ModelMap[foreignType]

        if(!foreignKey) {
          cb()
          return
        }
        var model = ForeignModel.findOne(foreignKey)
        //we couldn't find the model
        model.once("notExist", function(err) {
          model.save(function() {
            model.emit("load")
...
, assoc: function(assoc, callback) {
    ...
    } else if(this.polymorphicBelongAssociations[assoc]) {
      return this.polymorphicBelongAssociations[assoc](callback)
...
, validateField: function(field, callback, opts) {
    ...
    _.asyncEach(validations, function(value, rule, next) {
      if(rule === "min" && that.get(field) && that.get(field).length < value) {
        valid.push("short")
        next()
      } else if(rule === "isInt" && that.get(field) && typeof that.get(field) == 'number' && parseFloat(that.get(field)) == parseInt(that.get(field)) && !isNaN(that.get(field))) {
        valid.push("isInt")
        next()
