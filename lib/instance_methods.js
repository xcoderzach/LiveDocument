...
define( ["require", "./object_id", "underscore", "./utils"]
  ...
  , save: function(cb) {
      ...
      // Run this after all the beforeSave functions have
      // been called, unless any of them have errors
      var afterBeforeSave = function() {
        ...
        if(!that.persisted) {
          ...
          delete doc._id
          that.constructor.sendUpdateMessage({ _id: that.get("_id") }, doc, function(doc) {
            that.saved = true
            that.saving = false
            that.set(doc)
            cb(that)
      ...
      beforeSaves.push(_(afterBeforeSave).bind(that))
  ...
  , authorizeEdit: function(instance, done) {
      ...
      _(this.constructor.editUsers).each(function(associatedUser) {
        console.log(that.get("_id"), that.constructor.currentUserId)
