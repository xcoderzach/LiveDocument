...
, save: function(cb) {
    ...
    beforeSaves = beforeSaves.concat(
      function(inst, cb) {
        that.validate(inst, cb, {silent: true})
      }
      , _(afterAfterSave).bind(that)
    )
...
, validate: function(instance, cb, opts) {
    cb = cb || function() {}
    opts = opts || {}
    var that = this
      , valid = {}
    ...
    _.asyncEach(this.constructor.validations, function(validations, field, outerNext) {
      _.asyncEach(validations, function(value, rule, next) {
        if(rule === "min" && that.get(field).length < value) {
          ...
          next()
        } else if(rule === "max" && that.get(field).length > value) {
          valid[field] = valid[field] || []
          ...
          that.isUnique(field, that.get(field), function(unique) {
            if(!unique) {
              valid[field] = valid[field] || []
              valid[field].push("must be unique")
            }
            next()
          })
        } else {
