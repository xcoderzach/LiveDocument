...
Collection.prototype.sortBy = function(field, reverse) {
  this.removeAll()
  this.orderBy = field
  this.reverseSort = reverse || false
  this.reinsertAll()
  return this
}
...
Collection.prototype.insertAt = function(id) {
  ...
  if(this.orderBy) {
    insertIndex = _.sortedIndex(this.documents, id, function(idToCheck) {
      if(that.reverseSort) {
    ...
    insertIndex = this.documents.length
  }
  return insertIndex
}
...
Collection.prototype.doInsert = function(document) {
  ...
  if(index !== -1) {
    ...
    this.length = this.documents.length
    this.emit("insert", document, index, this)
  }
}
...
Collection.prototype.doRemove = function(document) {
  ...
  if(index !== -1) {
    ...
    this.length = this.documents.length
    this.emit("remove", document, index, this)
  }
}
