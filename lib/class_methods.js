...
var ClassMethods = module.exports = {}
...
ClassMethods.find = function(query, callback) {
  ...
  var db = this.getDb()
    ...
    , queryId = db.read(query, function(docs, method) {
        collection.handleNotification(docs, method)
      })
...
ClassMethods.getDb = function() {
  if(this.isServer) {
    if(this.embedded) {
      return this.prototype.db.embedded("", "", this.collectionName())
    } else {
      return this.prototype.db.collection(this.collectionName())
    }
  } else {
    if(this.embedded) {
...
ClassMethods.serverBeforeSave = function(fn) {
  // this is to ensure that hasn't been parsed out
...
ClassMethods.setupManyDependent = function(name) {
  this.serverAfterRemove(function(instance) {
    instance.assoc(name, function(items) {
