...
define( ["require", "underscore", "./live_document_collection"]
  ...
  LiveDocumentClassMethods.setSocket = function(socket) {
    ...
    this.socket.on("LiveDocumentChange", function(id, doc) {
      _(that.listeners[id]).each(function(instance) {
        instance.handleChange(doc)
      })
    })

    this.socket.on("LiveDocumentRemove", function(id) {
      _(that.listeners[id]).each(function(instance) {
        instance.handleRemove()
      })
    }) 
     
    this.socket.on("EmbeddedLiveDocumentCreate", function(parentName, id, associationName, embeddedDocumentId, doc) {
      _(that.listeners[id]).each(function(instance) {
        instance.manyAssociations[associationName].add(doc)
      })
    })

    this.socket.on("EmbeddedLiveDocumentRemove", function(parentName, id, associationName, embeddedDocumentId) {
      _(that.listeners[id]).each(function(instance) {
        instance.manyAssociations[associationName].remove(embeddedDocumentId)
      })
    }) 
  }
  ...
  */
  ...
  LiveDocumentClassMethods.sendDeleteMessage = function(query, callback) {
    this.socket.emit("LiveDocumentDelete", this.collectionName(), query, callback)
  }
  ...
  */
  ...
  LiveDocumentClassMethods.sendUpdateMessage = function(query, document, callback) {
    this.socket.emit("LiveDocumentUpdate", this.collectionName(), query, document, callback)
  }
  ...
  LiveDocumentClassMethods.readOne = function(query, callback) { 
    ...
    this.sendReadMessage(query, function(docs) {
      if(docs[0]) {
        ...
        throw new Error("No documents match the conditions provided")
      }
    })
    return doc
  }
