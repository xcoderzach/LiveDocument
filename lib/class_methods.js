...
define( ["require", "underscore", "./live_document_collection"]
  ...
  /**
    * The static methods, they need their own constructor though!
    **/
  var LiveDocumentClassMethods = {

    isServer: (process.platform !== "browser")
  ...
  , findOne: function(conditions, callback) { 
      ...
      var doc = new this({_id: conditions._id})
      this.sendReadMessage(conditions, function(docs) {
        if(docs[0]) {
  ...
  , scrub: function(documents, method) {
  ...
  , editableBy: function(associatedUser) {
